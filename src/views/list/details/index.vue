<template>
  <div class="top_box">
    ...
    <nut-row class="order-detail">
      ...
      <nut-col :span="24" class="order-circle">
        <nut-circle-progress progress="20" radius="60" color="#5460FE"> Used:
          {{ Math.round(orderInfo.value.used_space / orderInfo.value.total_space) * 100 }}% </nut-circle-progress>
      </nut-col>
  ...
  <div class="detail_box">
    ...
    <!-- share -->
    <nut-popup @closed="isReady = false" position="bottom" closeable round :style="{ height: '200px', 'z-index': 999 }"
    ...
    <nut-uploader :url="uploadUri" :timeout="1000 * 60 * 60" :before-upload="beforeupload" :disabled="isDisabled"
      :data="formData" :headers="formData" :before-xhr-upload="beforeXhrUpload" :xhr-state="successStatus"
      @success="uploadSuccess" @progress="onProgress" @start="onStart" @failure="onFailure" @change="onChange"
      ref="uploadRef" class="upload_class">
      <nut-button type="success" class="upload_btn" size="small">+</nut-button>
    </nut-uploader>
    <nut-dialog v-model:visible="dialogVisible" title="Custom Name" :close-on-click-overlay="false" :show-cancel="false"
      :show-confirm="false" class="CustomName">
...
<style lang="scss">.order-circle .nut-circle-progress {
  svg {
    border-radius: 50%;
