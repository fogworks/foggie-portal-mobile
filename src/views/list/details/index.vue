<template>
  <div class="top_box">
    ...
    <nut-row class="order-detail">
      ...
      <nut-col :span="24" class="order-circle">
        <nut-circle-progress :progress="((orderInfo.value.used_space || 0) / (orderInfo.value.total_space || 1)) * 100"
          radius="60" color="#5460FE">
  ...
  <div class="detail_box">
    ...
    <div class="today_file">
      ...
      <span class="see_all"
        @click="router.push({ name: 'FileList', query: { ...route.query, category: 0, bucketName } })">See All ></span>
    </div>
    ...
    <nut-infinite-loading load-more-txt="No more content" v-if="tableData.length" :has-more="false" class="file_list">
      <div @click="handleRow(item)" :class="['list_item']" v-show="index < 4" v-for="(item, index) in tableData"
        :key="index">
    ...
    <Teleport to="body">
      <nut-overlay overlay-class="detail_over" v-if="detailShow" v-model:visible="detailShow"
    ...
    <!-- share -->
    <Teleport to="body">
      <nut-popup v-if="showShareDialog" @closed="
        isReady = false;
      shareType = '';
      " position="bottom" closeable round :style="{ height: '300px' }" v-model:visible="showShareDialog">
        <div v-if="isReady" class="rename_box move_box">
          <nut-cell style="margin-top: 50px" title="Access Period:">
            <template #link>
              <span style="display: flex">{{ desc }} <IconEdit style="margin-left: 5px; color: #abacff"
                  @click="periodShow = true"></IconEdit></span>
            </template>
          </nut-cell>
          ...
          <nut-popup position="bottom" v-model:visible="periodShow">
            <nut-picker v-model="periodValue" :columns="options" title="Select expiration time" @confirm="confirmPeriod"
              @cancel="periodShow = false">
            </nut-picker>
          </nut-popup>
          <nut-button type="info" block
            @click="() => confirmHttpShare(shareType, detailRow.value, accessKeyId, secretAccessKey, bucketName)">Confirm</nut-button>
        </div>
    ...
    <nut-dialog v-model:visible="dialogVisible" title="Bucket Name" :close-on-click-overlay="false" :show-cancel="false"
      ...
      <p class="bucket_tip" style="text-align: left; word-break: break-word">Buckets are used to store and organize your
        files.Custom names can only contain lowercase letters, numbers, periods, and dashes
      ...
      <nut-input v-model="newBucketName" placeholder="Please enter Custom Name" max-length="10"
        min-length="8"></nut-input>
      <template #footer>
  ...
  <uploader v-if="isMobileOrder" :bucketName="bucketName" :accessKeyId="accessKeyId" :secretAccessKey="secretAccessKey"
    :orderInfo="orderInfo" @uploadComplete="uploadComplete"></uploader>
