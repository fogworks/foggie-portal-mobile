      ...
      >
        ...
        <div class="ipfs" v-if="chooseItem.isPin || chooseItem.cid">
          ...
          <p v-if="chooseItem.cid">
            <span>{{ chooseItem.imgUrlLarge }}</span>
...
<script setup lang="ts">
  ...
  import { poolUrl, browserUrl, maxUrl } from '@/setting.js';
  ...
  async function getFileList(scroll: string, prefix: any[], reset = false) {
  ...
  const confirmRename = () => {
    ...
    if (isNewFolder.value) {
      ...
      server.touchFile(ProxFileInfo, metadata, (err: { message: any }, data: any) => {
        if (data) {
      ...
      server.renameObjects(ProxRenameObject, metadata, (err: { message: any }, data: any) => {
        if (data) {
