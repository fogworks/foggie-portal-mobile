  ...
  const loadMore = async (
    ...
    await search_max({
      ...
      for (let i = 0; i < arr.length; i++) {
        let item = arr[i];
        let requestTarget = {
          dedicatedip: item.ipaddress || item.dedicatedip,
          device_id: item.device_id,
        };
        if (arr[i].deploy_svc_gateway_state === 'initializing') {
          //  currentDevice.value = arr[i];
          //  activeStep.value = 2;
          //  showProgressWrap.value = true;
          //  isActiveLoading.value = true;
          //  startloading();
        }
        // let res = await get_vood_token({ vood_id: requestTarget.device_id });
        if (res) {
          //  store.dispatch('token/setTokenMap', {
          //    id: requestTarget.device_id,
          //    token: res.data.token_type + ' ' + res.data.access_token,
          //  });
        }
        // try {
        //   let respon = await get_service_info(requestTarget);
        //   console.log(respon, maxTableData.value[i].deploy_svc_gateway_state);
        //   let status = respon && respon.result && respon.result.svc_state;
        //   // item.deploy_svc_gateway_state = status;
        //   maxTableData.value[i].deploy_svc_gateway_state = status;
        //   // maxTableData.value.push(item);

        //   // maxTableData.value[i].deploy_svc_gateway_state = status;
        //   // console.log(maxTableData.value[i].deploy_svc_gateway_state, status, "9999");
        // } catch (error) {
