  ...
  <div style="height: auto; padding-bottom: 50px">
    ...
    <div class="top_back_line">
      <div class="inside_blue">
        <div class="back_icons" @click="shopBack">
      ...
      <div class="circle_box circle_box_change">
        <div class="change_left">
          <nut-form class="query_form" :model-value="shopForm">
            <nut-form-item label="">
              <nut-input v-model="shopForm.quantity" />
            </nut-form-item>
          </nut-form>
          <span>GB</span>
        </div>
        <div class="change_icon"> <img src="@/assets/tg/shop_change.svg" /></div>
        <div class="change_right">
          <span class="value_text">{{ Number(1 * 1 * shopForm.quantity).toFixed(4) }}</span>
          <span>DMCX</span>
        </div>
      </div>
    ...
    <div class="space-info" v-if="currentPage === 'stake'">
      ...
      <p @click="copySecret(calc_price.total)">
        <span class="p_label">Need Stake:</span><span>{{ calc_price.total }} DMCX<IconCopy color="#2f95eb;"></IconCopy></span>
      </p>
      <p @click="copySecret(address)">
        <span class="p_label">Current account: </span
        ><span>{{ address ? `${address.substr(0, 6)}...${address.substr(-6)}` : address }} <IconCopy color="#2f95eb;"></IconCopy></span>
      </p>
      <p @click="copySecret(calc_price.to)">
        ...
        ><span
          ...
          <IconCopy color="#2f95eb;"></IconCopy
        ></span>
      </p>
      <div class="my_canvas_box">
        <canvas ref="qrCanvas"></canvas>
        <div style="padding: 4px 0px; font-size: 10px; font-style: italic; font-weight: bold; color: #fff" class="bottom_tips_line">
      ...
      <hr />
      <div style="padding: 4px 0px; font-size: 11px; font-style: italic; font-weight: bold; color: #fff" class="bottom_tips_line">
        <img src="@/assets/tg/shop_tips.svg" class="warn_svg" />
    ...
    <div class="space-info unpay_item_wrap" v-if="currentPage === 'unPayItem'">
      ...
      <p @click="copySecret(currentStakeItem.dmcx_price)">
        <span class="p_label">Need Stake::</span><span>{{ currentStakeItem.dmcx_price }} DMCX<IconCopy color="#2f95eb;"></IconCopy></span>
      </p>
...
<style lang="scss" scoped>
  ...
  .vip_order_choose {
    border: 5px solid #f3d1a2;
    border: 0.666667vw solid #2f95eb;
    margin: 20px;
    border-radius: 10px;
    background: #fff;
