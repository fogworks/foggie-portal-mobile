<template>
    <div class="qrcode">
        ...
        <nut-popup :safe-area-inset-bottom="true" position="bottom" closeable round pop-class="scanPopup" z-index="2001"
            :style="{ minHeight: '40%', height: 'auto' }" v-model:visible="isPopupShow">
            <template v-if="total > 0">
                <div class="title">Do you want to log in? Your OOD has the following options, please select the
                    authorization login</div>
                <div class="total_tag">
                    <nut-tag>{{ total }}</nut-tag> Buckets
                </div>
                <nut-infinite-loading ref="listRef" load-more-txt="No more content" v-model="infinityValue"
                    :has-more="hasMore" @load-more="loadMoreFun" class="bucket_box">
                    <div class="loadBox">
                        <div class="bucket_item" @click="ConfirmAuthorization(item)" v-for="item in listData"
                            :key="item.order_id">
            ...
            <template v-else>
                <div class="title">Do you want to log in? Currently, you don't have any available OOD. Please proceed to
                    purchase.</div>
                <router-link to="shop">
...
async function changeFile(event) {
    ...
    switch (html5QrCode.value && html5QrCode.value.getState()) {
        case 1:
            //未开始
            console.log('未开始');
            break;
    ...
    html5QrCode.value.scanFile(file, true)
        .then((decodedText) => {
