<template>
  <div class="qrcode">
    ...
    <nut-popup :safe-area-inset-bottom="true" position="bottom" closeable round pop-class="scanPopup" z-index="2001"
      :style="{ height: '80%' }" v-model:visible="isPopupShow">
      <template v-if="total > 0">
        <div class="title">Do you want to log in? Your OOD has the following options, please select the authorization
          login</div>
        <div class="total_tag">
          <nut-tag>{{ total }}</nut-tag> Buckets
        </div>
        <nut-infinite-loading ref="listRef" load-more-txt="No more content" v-model="infinityValue" :has-more="hasMore"
          @load-more="loadMoreFun" class="bucket_box">
          <div class="loadBox">
      ...
      <template v-else>
        <div class="title">Do you want to log in? Currently, you don't have any available OOD. Please proceed to purchase.
        </div>
        <router-link to="shop">
...
@keyframes scanAnimation {
  ...
  100% {
    top: 80vh;
    opacity: 0;
    background: linear-gradient(to right, rgba(0, 0, 255, 0) 0%, rgb(113, 113, 237) 50%, rgba(0, 0, 255, 0) 100%);
    /* 蓝色渐变效果 */
  }
}
</style>
